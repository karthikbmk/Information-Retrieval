...
def create_index(tokens):
    ...
    for doc_id,doc in enumerate(tokens):
        for word in doc:
...
def intersect(list1, list2):
    ...
    while (list1_ptr < len1 and list2_ptr < len2):
        if list1[list1_ptr] == list2[list2_ptr]:
            intersect_list.append(list1[list1_ptr])
            list1_ptr = list1_ptr + 1
...
def sort_by_num_postings(words, index):
    ...
    for word in words:
        final_dict[word] = len(index[word])
    
    return [tuple[0] for tuple in sorted(list(final_dict.items()), key=operator.itemgetter(1))]
...
def search(index, query):
    ...
    """
    ###TODO  
    wordsList = sort_by_num_postings(tokenize(query), index)
    
    docsList = index[wordsList[0]]
    
    for word in wordsList[1:]:
        docsList = intersect(docsList, index[word])
