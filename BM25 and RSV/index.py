...
class Index(object):
    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        for term,docId_tf_list in index.items():
            for docId_tf in docId_tf_list:
                if docId_tf[1] != 0 :                    
                    doc_length[docId_tf[0]] += (((1 + math.log10(docId_tf[1])) * math.log10(float(n_docs)/doc_freqs[term]))**2)
                else:
                    doc_length[docId_tf[0]] += 0
    ...
    def compute_doc_lengths(self, index):
        ...
        for term_tf_idf in index.values():
            for doc_tf_idf in term_tf_idf:
                doc_length[doc_tf_idf[0]] += doc_tf_idf[1]
                             
        return doc_length , float(sum(doc_length.values()))/len(doc_length)
    ...
    def create_tf_index(self, docs, doc_freqs):
        ...
        k_v = defaultdict(lambda : [])
        
        for d_id,doc in enumerate(docs):            
            for k,v in Counter(doc).items():
                k_v[k].append([d_id+1,v*1.0]) # Why floats ?
    ...
    def query_to_vector(self, query_terms):
        ...
        for term in query_terms:
            if term in self.index:                
                term_idf[term] = math.log10((len(self.documents)*1.0)/self.doc_freqs[term]) 
