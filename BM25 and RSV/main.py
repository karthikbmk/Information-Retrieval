...
def parse_relevance_strings(strings):
    ...
    for string in strings:
        q_r = re.findall('\w+',string)          
        if len(q_r) >= 2:     #To ensure new lines are not added.                       
            result[int(q_r[0])] = list(map(int,q_r[1:]))
...
def parse_query_strings(strings):
    ...
    for _id,string in enumerate(strings[start:]):
        
        if '*FIND' in string or '*STOP' in string:            
            k = strings[start - 1]                        
            k = re.sub('[^0-9]+','',k)                         
            res[int(k)] = (' '.join( filter(None, strings[start : _id]))).replace('\n \n','').strip()                        
            start = _id + 2 
...
def parse_document_strings(strings):
    ...
    """
    ###TODO
    text = [x.replace('\n','') for x in strings if x != '\n']
    
    start = 1
    res = []
    
    for _id,line in enumerate(text[start:]):
        
        if '*TEXT' in line or '*STOP' in line:
            res.append((' '.join(text[start:_id+1]).strip()))
            start = _id + 2
